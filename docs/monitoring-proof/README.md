...
Deployment verification on 7 August 2026 returned HTTP `200`, `status: ok`, `database: ok`, and version `f53f0d1`. Vercel function logs provide request/error visibility, while the `AIRequest` table records the AI use case, model, outcome (`resultCategory`), duration, and retry count for every request, without storing prompt bodies. Each row's own `id` is the traceable request identifier — there is no separate request-ID column.
