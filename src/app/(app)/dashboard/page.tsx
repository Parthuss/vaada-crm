...
export default async function DashboardPage() {
  ...
  const [leads, followUps] = await Promise.all([
    ...
    db.followUp.findMany({ where: { ownerId, completedAt: null, lead: { archivedAt: null } }, include: { lead: { select: { id: true, name: true, company: true } } }, orderBy: { dueAt: "asc" }, take: 8 }),
  ]);
  ...
  return <div className="page">
    ...
    <section className="grid dashboard-grid">
      <article className="card"><header className="card-head"><h2>Attention queue</h2><Link className="text-link" href="/follow-ups">View all <ArrowRight size={13} style={{ display: "inline" }} /></Link></header><div className="card-body attention-list">
        {attention.length ? attention.map((item) => <Link href={`/leads/${item.lead.id}`} className="attention-row" key={item.id}><span className={`dot ${item.bucket.toLowerCase()}`} aria-hidden /><div><strong>{item.lead.company}</strong><div className="subtle">{item.lead.name}</div></div><span className="hide-mobile">{item.note}</span><time className={`badge ${item.bucket.toLowerCase()}`}>{item.bucket === "OVERDUE" ? "Overdue · " : item.bucket === "TODAY" ? "Today · " : ""}{formatDateTime(item.dueAt)}</time></Link>) : <div className="empty"><strong>No open promises.</strong>Add a follow-up from any lead to build your daily queue.</div>}
      </div></article>
      <article className="card"><header className="card-head"><h2>Pipeline shape</h2><Link className="text-link" href="/pipeline">Open board</Link></header><div className="card-body status-stack">{stages.map((status) => { const count = leads.filter((lead) => lead.status === status).length; return <div className="status-line" key={status}><span>{status.charAt(0) + status.slice(1).toLowerCase()}</span><div className="bar"><i style={{ width: `${count / maximum * 100}%` }} /></div><strong>{count}</strong></div>; })}</div></article>
    </section>
  </div>;
}
