...
export function apiError(error: unknown, requestId = crypto.randomUUID()) {
  if (error instanceof ZodError) {
    return Response.json({ error: { code: "VALIDATION_ERROR", message: error.issues[0]?.message ?? "Check the submitted fields", fields: error.flatten().fieldErrors }, requestId }, { status: 400 });
  }
